...
from Layers import BatchNormalization, Dropout, RNN, Sigmoid, TanH
import numpy as np
...
class TestBatchNorm(unittest.TestCase):
    ...
    def test_update(self):
        ...
        for _ in range(10):
            ...
            layer.backward(error_tensor)
            new_output_tensor = layer.forward(self.input_tensor)
            self.assertLessEqual(np.sum(np.power(output_tensor, 2)), np.sum(np.power(new_output_tensor, 2)))
